	 ...
	 always @* begin
		case (pr_state)
			...
			HIGH1: begin                // 100.
				...
				ADC_OUT     <= ADC_OUT_reg;                                 // Capture SDO in the following cycle (master clock) because the SCLK output is registered, which is what the ADC sees and uses to output its data accordingly.    
				sample_cnt  <= sample_cnt_reg;
				tmax	    <= 1;
			...
			CAPTURE_SDO: begin          // 101. The 'REGISTERED' SCLK output transitions to a logic high. Capture SDO here. 
			    SYNCb       <= 0;
			    SCLK        <= 1;
			    SDI         <= SPI_DATA[FRAME_SIZE - 1 - pulse_cnt_reg];
